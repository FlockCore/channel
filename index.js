var dns = require('@flockcore/server')
...
function Revelation (opts) {
  if (!(this instanceof Revelation)) return new Revelation(opts)
  if (!opts) opts = {}
  ...
  if (this._whoami) {
    ...
    debug('not running a whoami() - DNS Revelation was not enabled')
  }
  ...
  function ondhtpeer (peer, infoHash, via) {
    ...
    if (via) debug('chan=%s dht revelation peer=%s:%s via=%s:%s', prettyHash(id), peer.host, peer.port, via.host || via.address, via.port)
    else debug('chan=%s dht revelation peer=%s:%s', prettyHash(id), peer.host, peer.port)
  ...
  function ondnspeer (name, peer) {
    ...
    debug('chan=%s dns revelation peer=%s:%s', prettyHash(id), peer.host, peer.port)
...
util.inherits(Revelation, events.EventEmitter)

Revelation.prototype.join = function (id, port, opts, cb) {
...
Revelation.prototype.leave = function (id, port) {
  if (this.destroyed) return
  if (!port) port = 0
  if (typeof id === 'string') id = bufferFrom(id)
...
Revelation.prototype.update = function () {
  var all = this.list()
...
Revelation.prototype.list = function () {
...
Revelation.prototype.destroy = function (cb) {
  if (this.destroyed) {
